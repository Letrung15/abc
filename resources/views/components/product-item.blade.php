<style>
    ...
    width: 100%;
    height: 0;
    padding-bottom: 100%; /* Tạo một khung hình vuông */
    position: relative;
    overflow: hidden;
}
...
<div class="m-1 border text-center position-relative">
    ...
    @endif
    <div class="box-hover image-container">
        <a id="wishlist_product-url{{ $product->id }}" href="{{ route('slug.index', ['slug' => $product->slug ?? '']) }}">
            <img class="img-change img-product img-fluid" src="{{ asset('images/product/'.$product->images[0]->image) }}" id="wishlist_product-image{{ $product->id }}" alt="{{ $product->images[0]->image }}" />
            @if (count($product->images) >= 2)
                <img class="img-product img-fluid" src="{{ asset('images/product/'.$product->images[1]->image) }}" alt="{{ $product->images[1]->image }}" />
            @endif
        </a>
    </div>
